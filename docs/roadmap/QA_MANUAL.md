...
| **Integration (requires Postgres)** | Dark web: free tier gated **`GET /api/dark-web/*`**; paid tier finding + **`GET /api/dashboard/metrics`** **`darkWebAlerts`** + dismiss; **`POST /api/dark-web/seed-demo`** **403** in **`NODE_ENV=production`** | `src/api/src/darkWeb.integration.test.ts` |
...
- [ ] **Onboarding:** 8-step modal appears until completed or **Skip**; steps follow **welcome → install extension → sign in → first alias → inbox → vault → brokers → billing (optional Pro)**; **Back** / **Next** / **Done**; **Next** navigates to the matching in-app route where applicable (Chrome Web Store / load-unpacked copy on the install step).
...
- [ ] **Quick actions** on overview: same routes plus **Dark web** and **Settings**; keyboard **Tab** shows visible focus rings.
- [ ] **Overview demo metrics (optional):** with API env **`DASHBOARD_DEMO_METRICS=1`** or **`OVERVIEW_DEMO_METRICS=1`**, Sword / weekly chart numbers are synthetic and the dashboard shows a **demo** banner (`metricsDemoMode`). **Unset** env vars for honest Phase 1 zeros in staging reviews.
...
- [ ] **Billing:** start Checkout (Stripe test mode on staging), complete test card; confirm return to **`/billing?session_id=…`** (or **`?canceled=`** if user abandons) behaves as implemented — tier sync from **`POST /api/billing/sync-checkout-session`** and UI shows **paid** when applicable (webhook may follow slightly later).
...
- [ ] **Vault:** with two browsers/sessions, provoke a sync conflict (edit vault on both); confirm **Retry sync** recovers after merge (LWW in `@phantom/shared`). **`PUT /api/vault/sync`** uses an **atomic** version check (`vaultSyncVersion` must match **`clientVersion`**) so concurrent writes cannot silently overwrite — expect **409** `sync_conflict` on mismatch, then client refetch + merge + retry (see `executeVaultSyncPush` tests).
...
- [ ] **Dark web (Pro):** **`/dark-web`** — paid tier sees findings list + **Check exposures** (HIBP when **`DARK_WEB_HIBP_API_KEY`** is set); free tier sees upgrade path. **`POST /api/dark-web/seed-demo`** is **403** in **`NODE_ENV=production`** (same rule as notification seed).
- [ ] **Notifications:** TopBar **bell** opens dropdown — loading, empty, error + **Retry**, and **Mark all read**; unread badge matches **`GET /api/notifications/count`** (poll **30s**). **`GET/PUT /api/notifications/preferences`** — toggle a category off; confirm **`GET /api/notifications`** + **`/count`** omit that category (rows may still exist in DB); **read-all** only marks unread in **enabled** categories. **Settings → Desktop notifications:** request permission; when **granted**, a **desktop** notification appears only when **unread count increases** (not every poll). **`POST /api/notifications/seed-demo`** is **403** in **`NODE_ENV=production`** (demo seed is dev/staging only).
...
| **503** (maintenance / overload) | Stop API or return **503** from a route | “Temporarily unavailable” style copy; responses include **`X-Request-Id`**. In **non-production**, some JSON errors also include **`error.requestId`** (same value). |
