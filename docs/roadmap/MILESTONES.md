...
| ------------------------ | ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| M1: Infrastructure Ready | Month 1     | **66%**  | API server, database, auth system deployed. CI/CD pipeline green. **`INFRA_AWS_PHASE1.md`** + **`DEPLOYMENT.md`** health/prod parity, **docker-compose** env table, **X-Request-Id** + structured errors, Redis rate-limit/session policy.                                                                           |
| M2: Vault Operational    | Month 2     | **52%**  | Client-side encryption working. Extension ↔ dashboard vault sync (**Last synced**, conflict hint). Vault page with card grid, strength meter, generate modal, search/filter. |
| M3: Extension MVP        | Month 2     | **63%**  | Form detection + **`fetchAuth`** offline **`network_error`** + **`refreshSession`** **503** retry (tests). Alias generation, autofill via Shadow DOM shield icon.                 |
| M4: Dashboard MVP        | Month 3     | **65%**  | Login, alias list, overview **Get started** + **Quick actions** routes, broker **status legend**, onboarding **billing** step, notification center, **inbox** read/unread + filter.                                    |
...
| M6: Broker Scanner       | Month 4     | **69%**  | 150+ brokers searchable. Scan results display in dashboard. Rate-limited workers + optional **`BROKER_SCAN_CONCURRENCY`**; summary exposes removal eligibility (`canRequestRemoval`). |
| M7: Broker Removal       | Month 5     | **57%**  | Simulated removal for paid tier; DIY + **API-method** labeled auto-queue (sim); results **status legend**; tier-gated queue; **`FREE_TIER_BROKER_SCAN_MAX_PER_24H`** for scan frequency. |
| M8: Free Tier Launch     | Month 6     | **59%**  | Public launch path: **`DEPLOYMENT.md`** + **`INFRA_AWS_PHASE1.md`**, **`/health/live`** + **`/health`**, free-tier scan cap, CI; **`EXTENSION_STORE_BUILD.md`** (manifest path + prod parity table); **`CHROME_WEB_STORE_CHECKLIST.md`**; **`QA_MANUAL.md`** billing + vault + Stripe duplicate steps. CWS listing + marketing (external). |
| M9: Paid Tier Launch     | Month 6     | **62%**  | Stripe Checkout + Portal; **`POST /api/webhooks/stripe`** persists **`StripeWebhookEvent`** (`event.id`) for idempotent retries + **`duplicate: true`**; **`POST /api/billing/sync-checkout-session`** (post-redirect tier sync) → **`User.tier`**. Integration tests in CI (incl. duplicate `event.id`); manual billing QA in **`QA_MANUAL.md`**. Live Stripe keys + webhook URL remain an external cutover. |
