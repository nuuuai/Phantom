...
| ------------------------ | ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| M1: Infrastructure Ready | Month 1     | **82%**  | API + **Postgres** + **Redis** (compose + CI **Redis 7** service + **`REDIS_URL`**); **JWT** HS256/RS256 (`jwt.ts`, **`clockTolerance`**, explicit verify **algorithms**); refresh hash + rotation + **`authSession.integration.test.ts`**; **`DEPLOYMENT.md`** JWT/Redis accuracy; **`AUTH_AND_VAULT_PHASE1.md`** JWT + refresh rows. **`INFRA_AWS_PHASE1.md`** + **`infra/terraform/`** + CI **`terraform validate`**; **X-Request-Id** + structured errors.                                                                           |
...
| M3: Extension MVP        | Month 2     | **81%**  | Form detection + open **shadow root** scan + native value setter for controlled inputs; **`fetchAuth`** / **`refreshSession`**: **`network_error`** vs API **503** vs **401**; exponential backoff on refresh (tests). Shared **`@phantom/shared`** **`clientError`** mapping on login / generate. Alias generation (**popup** **Email**/**Password** + **`fieldKind`**), **shield** autofill + **visible errors** + **`InputEvent`** + fixed viewport positioning (scroll/resize). Popup **`role="alert"`** on error paths.                 |
| M4: Dashboard MVP        | Month 3     | **89%**  | Same funnel + **NotificationCenter** (bell, **unread delta** desktop **`Notification`** when granted), **Settings** as account hub: parallel **`user/me`** + **`GET /api/notifications/preferences`**, **forward-to** + API **400** surfaces, quotas + **Aliases**/**Vault** links (paid: **`max` null** / unlimited copy), **Billing** + **UpgradeModal**, prefs **PUT** + **invalidate**, per-section **Retry**, desktop permission copy (**default**/**granted**/**denied**), **Danger zone** sign-out (no delete-account API). API **read-path** prefs filter (**`notifications.ts`** doc); **`POST /notifications/seed-demo`** **403** in production; **`notifications.integration.test.ts`** (prefs hide category, seed **403**, bad **`enabled`**) with Postgres. **`emailInbox.integration.test.ts`** (webhook → list → patch). **`parseApiResponseJson`** **`httpStatus`**; inbound webhook **401/415/503** tests. **Run 21:** **MobileNavBar** (narrow viewports) + sidebar from **md** breakpoint, **`titleForPath`** alias detail (**Retry** + email forwarding section), **7-step onboarding** (**install extension → first alias** → … → billing, **Back**, CWS/dev copy, **`DASHBOARD_PATHS`**), **Quick actions** + **Get started** + upgrade → **`/billing`**, skip link, responsive padding. **Run 22:** **Aliases** list **Retry** + **`GenerateAliasModal`** per-type quota; **`aliasTierLimits`** username cap test. **Run 23:** shared **`UpgradeModal`** + **`upgradeCopy`** (single string table); **`BrokersPage`** / alias / vault / onboarding billing step; **`BillingPage`** FAQ + checkout success. **Vault** list error **Retry** + paid unlimited header. **Run 26:** **`SettingsPage`** hub + **`QA_MANUAL`** / **`DEPLOYMENT`** notes.                                    |
...
| M7: Broker Removal       | Month 5     | **68%**  | Simulated removal for paid tier (**`BROKER_REMOVAL_QUEUE.md`** + env note); **`brokerRemovalMethodLabel`** (API vs DIY) + row CTAs + free **Upgrade** copy; results **status legend**; tier-gated queue (**`upgrade_required`** → shared **`UpgradeModal`** / **`upgradeCopy`**); **`FREE_TIER_BROKER_SCAN_MAX_PER_24H`** on API summary + UI; **429** scan cap banner + **`/billing`** path; read-path **`advanceRemovalSimulation`**. **Run 23:** honest simulated labels; API **`code`** → UI **reason** mapping tested. |
| M8: Free Tier Launch     | Month 6     | **80%**  | Public launch path: **`DEPLOYMENT.md`** (extension **`host_permissions`** vs **`CORS_ORIGIN`**, options override) + **`INFRA_AWS_PHASE1.md`**, **`/health/live`** + **`/health`**, free-tier scan cap + alias **`tier_limit`** body + **integration test** for **429**, CI (**`npm ci` → migrate → seed → lint → test → build** → **`terraform validate`**, Postgres **16** + Redis **7**, **`cancel-in-progress`**); **`EXTENSION_STORE_BUILD.md`** (MV3 zip path, **`onInstalled`**); **`CHROME_WEB_STORE_CHECKLIST.md`** (permissions table, **`npm run build:extension:store`** in checklist, **Run 10** manifest verify); **`QA_MANUAL.md`** tier matrix + **free → `/billing` → tier refresh** (Run 23) + billing query params (`session_id`, `canceled`) + vault + extension + **stakeholder handoff** paragraph. **`npm run build:extension:store`** prod zip. **Shipped in repo:** MV3 least-privilege manifest, API URL validation + options, expanded unit tests. **`NOTIFICATIONS_EMAIL_ENABLED`** doc honesty (not read by API). **Run 10:** external blockers table (**owner + blocker**), Phase 2 **docs-only** handoff. **Blocked (external):** CWS account, hosted privacy policy URL, marketing. |
| M9: Paid Tier Launch     | Month 6     | **79%**  | Stripe Checkout + Portal; **`POST /api/webhooks/stripe`** persists **`StripeWebhookEvent`** (`event.id`) for idempotent retries + **`duplicate: true`**; **`POST /api/billing/sync-checkout-session`** → **`User.tier`** + **`subscriptionStatus`**; dashboard **`/billing?session_id=`** + **FAQ** / success after return; shared **upgrade** CTAs (**aliases**, **vault**, **brokers**, **onboarding billing step**) → same **`/billing`** funnel; **`buildAliasUsage`** + vault UI reflect **unlimited** password aliases ( **`max` null** ). Integration tests in CI (signed webhooks, mocked Stripe retrieve); **`QA_MANUAL.md`** tier matrix + billing steps aligned with code. **Run 10:** **`DEPLOYMENT.md`** external blockers (**owner + blocker**) for live Stripe webhook + dashboard URL. **Blocked (external):** Live Stripe keys + webhook URL on public HTTPS. |
