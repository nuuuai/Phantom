...
|------|--------|-------------------------|
| [PHASE_1_FOUNDATION.md](./PHASE_1_FOUNDATION.md) | Months 1–6, desktop MVP | Deliverables avg **~86%** (see table at top of file; updated per ship). |
...
- **Run 21 / dashboard shell & onboarding:** **`dashboardRoutes.ts`** (**`DASHBOARD_PATHS`**, **`QUICK_ACTIONS`**); **`MobileNavBar`** + hidden sidebar on small screens; **`OnboardingModal`** 7 steps, **Back**, scroll, Chrome Web Store + dev **load unpacked** + **`chrome-extension://`** limitation; **`routeTitles`** alias detail + tests; **`BrokerUpgradeModal`** → **`/billing`**; skip link **`#main-content`**; **`PHASE_1_FOUNDATION`** onboarding + web dashboard + **M4** bumps.
